...
SELECT email FROM User WHERE email LIKE '%@gmail.___' LIMIT 3; -- gmail with 3-letter TLD
SELECT seat_number FROM Seat WHERE seat_number LIKE 'A_' LIMIT 3; -- A followed by one character
...
-- Using & (BITWISE AND - though rarely used)
SELECT screen_id FROM Screen WHERE screen_id & 1 = 1 LIMIT 3; -- Odd numbered screens
SELECT movie_id FROM Movie WHERE movie_id & 3 = 0 LIMIT 3; -- IDs divisible by 4
SELECT booking_id FROM Booking WHERE booking_id & 1 = 0 LIMIT 3; -- Even numbered bookings
